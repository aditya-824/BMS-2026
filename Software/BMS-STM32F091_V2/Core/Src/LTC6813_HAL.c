...
uint16_t LTC6813_run_adc_overlap(uint8_t total_ic, // Number of ICs in the system
	...
	for (cic = 0; cic < total_ic; cic++) {
		...
		measure_delta = (int32_t) ic[cic].cells.c_codes[12]
				- (int32_t) ic[cic].cells.c_codes[13];
		if ((measure_delta > failure_pos_limit)
				|| (measure_delta < failure_neg_limit)) {
			error = error | (1 << (cic - 1));
		}
...
// REQURIE SERIAL TO UART PRINT CONVERSION
...
void LTC6813_set_discharge(int Cell, // Cell to be discharged
	...
	for (i = 0; i < total_ic; i++) {
		if (Cell == 0) {
			...
			ic[i].config.tx_data[4] = ic[i].config.tx_data[4]
					| (1 << (Cell - 1));
		} else if (Cell < 13) {
			ic[i].config.tx_data[5] = ic[i].config.tx_data[5]
					| (1 << (Cell - 9));
		} else if (Cell < 17) {
			ic[i].configb.tx_data[0] = ic[i].configb.tx_data[0]
					| (1 << (Cell - 9));
		} else if (Cell < 19) {
			ic[i].configb.tx_data[1] = ic[i].configb.tx_data[1]
					| (1 << (Cell - 17));
		} else {
			break;
		}
...
uint8_t LTC6813_rdpsb(uint8_t total_ic, //< number of ICs in the daisy chain
	...
	for (current_ic = 0; current_ic < total_ic; current_ic++) {
		...
		for (byte = 3; byte < 6; byte++) {
			ic[c_ic].sctrlb.rx_data[byte] =
					read_buffer[byte + (8 * current_ic)];
		}

		for (byte = 6; byte < 8; byte++) {
			ic[c_ic].pwmb.rx_data[byte] = read_buffer[byte + (8 * current_ic)];
			ic[c_ic].sctrlb.rx_data[byte] =
					read_buffer[byte + (8 * current_ic)];
		}
...
void LTC6813_set_cfgrb(uint8_t nIC, cell_asic *ic, bool fdrf, bool dtmen,
bool ps[2], bool gpiobits[4], bool dccbits[7]) {
...
void LTC6813_set_cfgrb_ps(uint8_t nIC, cell_asic *ic, bool ps[]) {
	...
	for (i = 0; i < 2; i++) {
		if (ps[i])
			ic[nIC].configb.tx_data[1] = ic[nIC].configb.tx_data[1]
					| (0x01 << (i + 4));
		else
			ic[nIC].configb.tx_data[1] = ic[nIC].configb.tx_data[1]
					& (~(0x01 << (i + 4)));
...
void LTC6813_set_cfgrb_gpio_b(uint8_t nIC, cell_asic *ic, bool gpiobits[]) {
	...
	for (i = 0; i < 4; i++) {
		if (gpiobits[i])
			ic[nIC].configb.tx_data[0] = ic[nIC].configb.tx_data[0]
					| (0x01 << i);
		else
			ic[nIC].configb.tx_data[0] = ic[nIC].configb.tx_data[0]
					& (~(0x01 << i));
